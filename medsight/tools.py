...
async def generate_summary_pdf(title: str, content: str, tool_context: ToolContext) -> dict:
    """Renders a Markdown medical summary into a styled, downloadable PDF artifact.

    Use this only when the user explicitly asks for a downloadable summary, a
    PDF, or something to save/print (e.g. after an image analysis or a medicine
    explainer). Pass `content` as Markdown — the tool formats it into a clean A4
    document: ## headings become section headings, `| tables |` become real
    tables, and - / 1. lists render as proper bulleted/numbered lists. Always
    keep the consult-a-clinician disclaimer inside the content.

    Args:
        title: Short title for the PDF (shown centered on the first page,
            and used as the filename), e.g. "Chest X-ray study summary".
        content: The full summary as Markdown.

    Returns:
        dict with 'status' and the artifact 'filename' the user can download.
    """
    ...
    safe_name = "".join(c if c.isalnum() else "_" for c in safe_title)[:50] or "summary"
