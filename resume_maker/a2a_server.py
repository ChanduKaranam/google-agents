"""A2A server for the Resume Maker agent — the deploy artifact Gemini Enterprise
...
* ``gen_ai_part_converter`` -> a catalog-aware :class:`A2uiPartConverter` so the
  model's validated ``send_a2ui_json_to_client`` output ships as an
  ``application/a2ui+json`` A2A DataPart (what GE renders), while normal text and
  file parts fall back to ADK's default converter.
* a ``before_agent`` interceptor that best-effort activates the A2UI A2A
  ...
  header today; A2UI still works because the toolset is enabled unconditionally.)

Run locally:
    uvicorn resume_maker.a2a_server:app --host 0.0.0.0 --port 8080
...
logger = logging.getLogger("resume_maker.a2ui")
...
AGENT_CARD = AgentCard(
    ...
    description=(
        ...
        or "ResumeAI — a guided, A2UI-powered resume builder and career coach."
    ),
    url=PUBLIC_URL,
    version="1.0.0",
    ...
    skills=[
        AgentSkill(
            ...
            name="Resume builder",
            description=(
                "Parse, score (ATS), rewrite and design a resume, then export a"
                " polished PDF — with tappable A2UI choices at each step."
            ),
            tags=["resume", "cv", "career", "ats", "a2ui"],
        )
    ],
)
...
def _gen_ai_part_converter(part):
    ...
    ``A2uiPartConverter.convert`` handles the ``send_a2ui_json_to_client`` tool
    output (returning ``application/a2ui+json`` DataParts) and A2UI text tags,
    and returns a list. ADK's executor expects a single ``Optional[Part]``, so we
...
def _is_leaked_tool_json(part) -> bool:
    """True if a Part is the raw ``send_a2ui_json_to_client`` tool output leaking
    (as text OR as a plain data part). The real UI already ships as an
    ``application/*a2ui`` DataPart, so this serialized copy would otherwise render
    as ugly JSON next to the chips. The legit A2UI DataPart is preserved because
    it carries an ``application/*a2ui`` mimeType and holds ``surfaceUpdate``
    directly (not wrapped in ``validated_a2ui_json``)."""
    root = getattr(part, "root", part)
...
def _executor_factory(runner) -> A2aAgentExecutor:
    config = A2aAgentExecutorConfig(
        ...
        execute_interceptors=[
            ...
            # PDF never reaches Gemini Enterprise, so "Generate PDF" comes back
            # empty. Not wired by to_a2a by default, and our custom interceptor
            # list replaces the default, so we add it explicitly.
            include_artifacts_in_a2a_event_interceptor,
        ],
    )
